...
class FileService {
    async getAllUserFiles(req){
        const {id} = req.params
        const files = await pool.query("select files.id, files.title,files.user_id, files.size, files.file_name, to_char(files.created_at, 'YYYY.MM.DD/HH24:MI') as created_at, json_build_object('id',users.id, 'username',users.username, 'avatar', users.avatar) as user from files inner join users on files.user_id = users.id  where user_id=$1", [id])
    ...
    async Userfilas(req){
        ...
        const files = await pool.query("select * from files where user_id=$1", [id])
    ...
    async getAllUsers(req){
        ...
        if (!search) {
            users = await pool.query("select files.id, files.title, files.size, files.file_name,  to_char(files.created_at, 'YYYY.MM.DD/HH24:MI') as created_at, json_build_object('id',users.id, 'username',users.username, 'avatar', users.avatar) as user from files inner join users on files.user_id = users.id")
        }else{
            users = await pool.query("select files.id, files.title, files.size, files.file_name,  to_char(files.created_at, 'YYYY.MM.DD/HH24:MI') as created_at, json_build_object('id',users.id, 'username',users.username, 'avatar', users.avatar) as user from files inner join users on files.user_id = users.id where files.title ilike $1", [`%${search}%`])
        }
    ...
    async getFile(req){
        ...
        const existVideo = [".mp4",".mkv",".avi",".mov",".webm",".flv",".wmv",".m4v",".3gp"];
        const existImage = [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp", ".svg", ".tiff", ".ico"];
        let filePath = ""

        if (media) {
            filePath = join(process.cwd(), "src","uploads", "media", file_name); 
        }else if (existVideo.includes(extname(file_name))) {
            filePath = join(process.cwd(), "src","uploads", "videos", file_name); 
        }else if(existImage.includes(extname(file_name))){
            filePath = join(process.cwd(), "src","uploads", "picture", file_name);
        }else{
            throw new BedRequestError(400, "Invalid file format")
        }
    ...
    async download(req){
        ...
        if (existVideo.includes(extname(file_name))) {
            filePath = join(process.cwd(), "src","uploads", "videos", file_name); 
        }else{
            throw new BedRequestError(400, "Invalid file format")
        }
